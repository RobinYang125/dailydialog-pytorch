...
class DataLoader(object):
    ...
    def dialogues_vpn_into_qas(self, dialogues, verbs, prons, nouns):
        qa_pairs = []
        verbs_list = []
        prons_list = []
        nouns_list = []
        ...
        return qa_pairs, verbs_list, prons_list, nouns_list
    ...
    def get_batch_test(self):
        ...
        qa_pairs, verbs_list, prons_list, nouns_list = \
            self.dialogues_vpn_into_qas(raw_data, verbs_raw_data, prons_raw_data, nouns_raw_data)
        ...
        return x_batch, x_v_batch, x_p_batch, x_n_batch, x_length, x_v_length, x_p_length, \
               x_n_length, go_y_batch, y_length, eos_map
...
class Generator(nn.Module):
    def __init__(self, embeddings):
        ...
        self.noun_output_layer = nn.Linear(self.hidden_size, self.sen_output_size)
        self.pron_output_layer = nn.Linear(self.hidden_size, self.sen_output_size)
        self.verb_output_layer = nn.Linear(self.hidden_size, self.sen_output_size)

        self.action_selector = nn.Linear(2 * self.hidden_size + 1, self.tag_size)
        self.state_bilinear = nn.Bilinear(self.hidden_size, self.hidden_size, 1)

        # self.noun_transform_layer = nn.Linear(100, self.sen_output_size)
        # self.pron_transform_layer = nn.Linear(100, self.sen_output_size)
        # self.verb_transform_layer = nn.Linear(100, self.sen_output_size)
        #
        # self.verb_bilinear = nn.Bilinear(self.sen_output_size, self.hidden_size, 100)
        # self.pron_bilinear = nn.Bilinear(self.sen_output_size, self.hidden_size, 100)
        # self.noun_bilinear = nn.Bilinear(self.sen_output_size, self.hidden_size, 100)

        # self.verb_transform = torch.randn((self.sen_output_size, self.sen_output_size),
        #                               requires_grad=True, dtype=torch.float32).cuda()
        #
        # self.pron_transform = torch.randn((self.sen_output_size, self.sen_output_size),
        #                               requires_grad=True, dtype=torch.float32).cuda()
        #
        # self.noun_transform = torch.randn((self.sen_output_size, self.sen_output_size),
        #                               requires_grad=True, dtype=torch.float32).cuda()

    def forward(self, verb_encoder_input, pron_encoder_input, noun_encoder_input,
                sen_encoder_input, target_word_input):
        ...
        for idx, word in enumerate(target_embed):
            ...
            #
            # m_r = self.state_bilinear(sen_hidden, hidden)
            #
            # a_s_i = torch.cat((sen_hidden, m_r, hidden), dim=-1)
            #
            # index_out = self.action_selector(a_s_i).view(1, -1)
            #
            # sen_out = self.sen_output_layer(output[0])
            # verb_out = self.verb_output_layer(output[0])
            # pron_out = self.pron_output_layer(output[0])
            # noun_out = self.noun_output_layer(output[0])
            #
            # stack_out = torch.stack([noun_out, verb_out, pron_out, sen_out]).view(1, 4, -1)
            # out = torch.mean(stack_out * index_out.view(1, -1, 1), dim=1)
            #
            # outputs.append(out)
            ...
            stack_out = torch.stack([noun_out, verb_out, pron_out, sen_out]).view(1, 4, -1)
            output_vocab = torch.mean(stack_out * index_out.view(1, -1, 1), dim=1)
...
model_prefix = 'models_' + prefix + '/generator_sgd_para_%s.pkl'
...
def main_train(is_toy=False):
    ...
    if is_toy:
        ...
        generator.load_state_dict(torch.load(model_prefix%'28'))
        # torch.backends.cudnn.enabled = False
        print('Model 28 has been loaded')
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        for bn in range(data_loader.batch_num + 1):
            x_batch, x_v_batch, x_p_batch, x_n_batch, x_length, x_v_length, x_p_length, \
               x_n_length, go_y_batch, y_length, eos_map = data_loader.get_batch_data()
            ...
            for idx, x_len_ele in enumerate(x_length):
                ...
                if is_toy:
                    x_batch_, x_v_batch_, x_p_batch_, x_n_batch_, go_y_batch_, eos_map_ = \
                ...
                outputs = generator(x_v_batch_, x_p_batch_, x_n_batch_, x_batch_, go_y_batch_)

                for jdx, eos_map_y_word in enumerate(eos_map_):
                    # print('word', torch.argmax(outputs[jdx]), eos_map_y_word.view(1,))
        ...
        for bn in range(data_loader.val_batch_num + 1):
            x_batch, x_v_batch, x_p_batch, x_n_batch, x_length, x_v_length, x_p_length, \
               x_n_length, go_y_batch, y_length, eos_map = data_loader.get_validation()
            if x_batch.all() == None:
                continue
        ...
        torch.save(generator.state_dict(), checkpoint_dir + 'generator_sgd_c_para_' + str(epoch + 1) + '.pkl')
...
def main_test(is_toy=False):
    ...
    res_file = 'models_' + prefix + '/vpn_rl_results.txt'
    res = codecs.open(res_file, 'w')

    reply_file = 'models_' + prefix + '/vpn_rl_reply.txt'
    reply_f = codecs.open(reply_file, 'w')

    ans_file = 'models_' + prefix + '/vpn_rl_answer.txt'
    ...
    checkpoint_file = 'models_' + prefix + '/generator_sgd_c_para_%s.pkl'
    if is_toy:
        generator = Generator(data_loader.embedding_matrix)
    else:
        generator = Generator(data_loader.embedding_matrix).cuda()
    ...
    for bn in range(data_loader.test_batch_num + 1):
        x_batch, x_v_batch, x_p_batch, x_n_batch, x_length, x_v_length, x_p_length, \
               x_n_length, go_y_batch, y_length, eos_map = data_loader.get_batch_test()
        if x_batch.all() == None:
            continue
...
if __name__ == '__main__':
    ...
    #     x_batch, x_v_batch, x_p_batch, x_n_batch, x_length, x_v_length, x_p_length, \
    #            x_n_length, go_y_batch, y_length, eos_map = d.get_batch_test()
