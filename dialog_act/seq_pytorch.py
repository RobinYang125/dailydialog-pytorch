...
class DAN_ACT(nn.Module):
    ...
    def forward(self, x_batch):
        ...
        rewards = torch.mean(attention, dim=-1)
...
model_prefix = 'models_seq2seq_pt/generator_para_%s.pkl'
...
def main_train(is_toy=False):
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        print("Epoch {:>3}/{} Training Loss {:g} - Valid Loss {:g}".format(epoch + 8,
                                    EPOCH_SIZE + 7, losses / step, val_losses / val_step))
        log.write("Epoch {:>3}/{} Training Loss {:g} - Valid Loss {:g}\n".format(epoch + 8,
                                    EPOCH_SIZE + 7, losses / step, val_losses / val_step))

        torch.save(generator.state_dict(), checkpoint_dir + 'generator_para_' + str(epoch + 8) + '.pkl')
        print('Model Trained and Saved in epoch ', epoch + 8)
