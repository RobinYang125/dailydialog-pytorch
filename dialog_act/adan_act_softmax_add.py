...
class DataLoader(object):
    ...
    def get_batch_test(self):
        ...
        return np.asarray(self.pad_sentence(sen_batch, np.max(sen_length))), np.asarray(sen_length), \
...
def main_train(is_toy=False):
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        for _ in range(data_loader.batch_num + 1):
            ...
            if is_toy:
                sen_batch, sen_length, act_label_ = torch.LongTensor(sen_batch),\
                                                    torch.LongTensor(sen_length),\
                                                    torch.LongTensor(act_label_)
            else:
                sen_batch, sen_length, act_label_ = torch.LongTensor(sen_batch).cuda(), \
                                                        torch.LongTensor(sen_length).cuda(), \
                                                 torch.LongTensor(act_label_).cuda()

            out, rew = model(sen_batch, sen_length)
        ...
        torch.save(model.state_dict(), checkpoint_dir + 'adan_sgd_para_' + str(epoch + 1) + '.pkl')
...
def main_test(is_toy=False):
    ...
    res_file = 'models_' + prefix + '/adan_act_softmax_pad_results.txt'
    ...
    checkpoint_file = 'models_' + prefix + '/adan_sgd_para_%s.pkl'
    if is_toy:
        model = DAN_ACT(data_loader.embedding_matrix)
    else:
        model = DAN_ACT(data_loader.embedding_matrix).cuda()
    ...
    for _ in range(data_loader.test_batch_num + 1):
        ...
        for idx, length in enumerate(sen_length):
            ...
            outputs, rewards = model(sen_batch, sen_length_, True)
