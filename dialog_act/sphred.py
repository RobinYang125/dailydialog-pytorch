...
class DataLoader(object):
    ...
    def get_sphred_case(self, first, second, reply):
...
def main_train(is_toy=False):
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        for bn in range(data_loader.batch_num + 1):
            ...
            outputs, z_mean, z_log_var = generator(pad_u1, u1_length_, pad_u2, u2_length_, go_pad_y, y_length_)
            ...
            kld_weight = min(sum_step / 10000, 1.0)
            ...
            for jdx in range(y_seq_len):
                ...
                loss_mean += torch.mean(
                    - 0.5 * torch.sum(
                        1 + z_log_var - torch.pow(z_mean, 2) - torch.exp(z_log_var)
                        , dim=-1)
                ) * kld_weight
                n_total += nTotal
...
def response_sphred(first, second, reply, is_toy=False):
    ...
    pad_u1_batch, u1_length, pad_u2_batch, u2_length, \
        eos_pad_y_batch, go_pad_y_batch, y_length = data_loader.get_sphred_case(first, second, reply)
    ...
    outputs, z_mean, z_log_var = generator(pad_u1, u1_length_, pad_u2, u2_length_, go_pad_y, y_length_, True)
...
if __name__ == '__main__':
    ...
    response_sphred('what for ?', 'i am going to read books', 'fine , ok')
