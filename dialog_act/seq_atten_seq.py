...
class Generator(nn.Module):
    ...
    def attention(self, hidden, encoder_outputs):
        ...
        # attn_energies = torch.zeros(batch_size, max_len).cuda() # B x S
        ...
        for b in range(batch_size):
            ...
            for i in range(max_len):
                attn_energies.append(self.score(hidden[b], encoder_outputs[b, i].unsqueeze(0)))
            attn_energies = torch.stack(attn_energies)
            self.final_attn.append(attn_energies)
        self.final_attn = torch.stack(self.final_attn).squeeze()

        # Normalize energies to weights in range 0 to 1, resize to 1 x B x S
        return F.softmax(self.final_attn, dim=0)
    ...
    def forward(self, encoder_input, target_input, is_test=False):
        ...
        encoder_outputs = torch.stack(encoder_outputs).view(1, -1, self.hidden_size)
        ...
        for idx, word in enumerate(target_embed):
            if is_test:
                decoder_output, hidden = self.docoder(emb.view(1, 1, -1), hidden)
                ...
                output_vocab = self.output_layer(concat_output[0])
                ...
                decoder_output, hidden = self.docoder(word.view(1, 1, -1), hidden)

                # [18, 1, 29]                    [18, 1, 300] [18, 29, 300]
                attn_weights = self.attention(decoder_output, encoder_outputs).view(1, 1, -1)
                context = attn_weights.bmm(encoder_outputs)
                concat_output = torch.cat((context, decoder_output), 2)

                outputs.append(self.output_layer(concat_output[0]))
...
def main_train(is_toy=False):
    ...
    if is_toy:
        ...
        # generator.load_state_dict(torch.load(model_prefix%'13'))
        # print('Model 13 has been loaded')
    generator_optimizer = torch.optim.Adam(generator.parameters(), lr=0.0001)
    # generator_optimizer = torch.optim.SGD(generator.parameters(), lr=0.0001)
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        torch.save(generator.state_dict(), checkpoint_dir + 'generator_sgd2_para_' + str(epoch + 1) + '.pkl')
...
def main_test(is_toy=False):
    ...
    res_file = 'models_' + prefix + '/seq_torch_seq_sgd_results.txt'
    res = codecs.open(res_file, 'w')

    reply_file = 'models_' + prefix + '/seq_torch_seq_sgd_reply.txt'
    reply_f = codecs.open(reply_file, 'w')

    ans_file = 'models_' + prefix + '/seq_torch_seq_sgd_answer.txt'
