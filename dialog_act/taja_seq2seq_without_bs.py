...
class TAJA_Seq2seq(object):
    ...
    def create_model(self):
        ...
        with tf.variable_scope(decoder, reuse=True):
            ...
            predict_output_array = tensor_array_ops.TensorArray(dtype=tf.int64,
                                                                      size=self.max_decoder_sequence_length,
                                             dynamic_size=False, infer_shape=True)

            def predicting_decode(i, hidden_state, decoder_outputs_array, inputs):
                overall_state = get_overall_state(hidden_state)
                cell_outputs, states = decoder_cell(inputs, (overall_state, ))
                outputs = output_layer(cell_outputs)
                output_id = tf.argmax(outputs, axis=-1)
                decoder_outputs_array = decoder_outputs_array.write(i, output_id)
                next_input_data = tf.nn.embedding_lookup(self.embedding_placeholder, output_id)
                return i + 1, states[0], decoder_outputs_array, next_input_data


            _, _, predict_output_array, _ = control_flow_ops.while_loop(
                cond=lambda i, _1, _2, _3: i < self.max_decoder_sequence_length,
                body=predicting_decode,
                loop_vars=(tf.constant(0, dtype=tf.int32), training_initial_state, predict_output_array,
                           tf.nn.embedding_lookup(self.embedding_placeholder, start_tokens))
            )

            final_outputs = tf.transpose(predict_output_array.stack(), perm=[1, 0])

        self.training_logits = tf.identity(training_decoder_output, name='training_logits')

        self.predicting_logits = tf.identity(final_outputs, name='predicting_logits')
...
def main_train(is_toy=False):
    ...
    log_file = 'log/' + prefix + '_log_latter20.txt'
    ...
    checkpoint_storage = prefix + '_models/checkpoint'
    checkpoint_dir = prefix + '_models/'
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(EPOCH_SIZE):
            ...
            print("Epoch {:>3}/{} Training Loss {:g} - Valid Loss {:g}".format(epoch + epoch_diff,
                                        EPOCH_SIZE + 50, losses / step, val_losses / val_step))
...
def main_test(is_toy=False):
    ...
    reply_file = checkpoint_dir + 'taja_reply.txt'
    reply_f = codecs.open(reply_file, 'w')

    ans_file = checkpoint_dir + 'taja_answer.txt'
    ans_f = codecs.open(ans_file, 'w')

    res_file = checkpoint_dir + 'results.txt'
    ...
    with test_graph.as_default():
        with tf.Session(config=config) as sess:
            ...
            checkpoint_file = prefix + '_models/model-70'
            # checkpoint_file = prefix + '_models/model-53'
