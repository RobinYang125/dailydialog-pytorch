...
class DAN_ACT(nn.Module):
    ...
    def __init__(self, embeddings):
        ...
        self.top_hidden_layers = nn.Sequential(nn.Linear(self.embedding_size, layer1_units),
                                    nn.Dropout(self.drop_out))

        self.output_layer = nn.Sequential(nn.Linear(layer1_units, self.act_class_num),
                                    nn.Dropout(self.drop_out))

        self.inter_h_emb = nn.Bilinear(self.embedding_size, self.embedding_size, 1)

    def mean(self, ten, lengths, dim=0):
        mean_res = []
        for idx, batch in enumerate(ten):
            res = torch.mean(batch[:lengths[idx], :], dim=dim)
            mean_res.append(res)
        final_mean = torch.stack(mean_res).view(len(lengths), -1)
        return final_mean

    def forward(self, x_batch, x_lengths, is_test=False):
        embed = self.embedding_layer(x_batch)
        dense_inputs = self.mean(embed, x_lengths)
        ...
        reward = []
        for e_w_id in range(seq_len):
            embed_word = embed[:, e_w_id, :]
            a_i = self.inter_h_emb(embed_word.view(-1, self.embedding_size), mid_rep)
            reward.append(a_i)

        reward = torch.stack(reward).view(batch_size, -1)
        reward = F.softmax(reward, dim=1)
        addition = self.mean(reward.view(batch_size, -1, 1) * embed, x_lengths)

        top_hidden = self.top_hidden_layers(addition)

        output = self.output_layer(top_hidden)
...
class Generator(nn.Module):
    ...
    def forward(self, encoder_input, target_input, is_test=False):
...
act_prefix = 'models_adan_act_softmax_add/adan_sgd_para_%s.pkl'
...
def main_train(is_toy=False):
    ...
    if is_toy:
        ...
        act.load_state_dict(torch.load(act_prefix%'100'))
    else:
        act = DAN_ACT(data_loader.embedding_matrix).cuda()
        act.load_state_dict(torch.load(act_prefix%'3'))
    ...
    if is_toy:
        ...
        generator.load_state_dict(torch.load(model_prefix%'4'))
        print('Model 4 has been loaded')
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        for bn in range(data_loader.batch_num + 1):
            pad_x_batch, x_length, eos_pad_y_batch,  go_pad_y_batch, y_length, \
                            x_act_batch, y_act_batch = data_loader.get_batch_data()
            ...
            for idx, x_len_ele in enumerate(x_length):
                ...
                # act
                act_outputs, act_rewards = act(pred_id_tensor, pre_len, True)

                sen_reward = act_outputs[0][y_act]

                final_act_rewards = act_rewards[0] * sen_reward


                for jdx in range(pre_len[0]):
        ...
        for _ in range(data_loader.val_batch_num + 1):
            pad_x_batch, x_length, eos_pad_y_batch,  go_pad_y_batch, y_length, \
                            x_act_batch, y_act_batch = data_loader.get_validation()
            if pad_x_batch.all() == None:
                continue
...
def main_test(is_toy=False):
    ...
    res_file = 'models_' + prefix + '/seq_dan_wsr_results.txt'
    res = codecs.open(res_file, 'w')

    reply_file = 'models_' + prefix + '/seq_dan_wsr_reply.txt'
    reply_f = codecs.open(reply_file, 'w')

    ans_file = 'models_' + prefix + '/seq_dan_wsr_answer.txt'
    ...
    for _ in range(data_loader.test_batch_num + 1):
        ...
        for idx, x_len_ele in enumerate(x_length):
            ...
            outputs = generator(pad_x, go_pad_y, True)

            reply = []
            for jdx, ele in enumerate(outputs):
                id_word = torch.argmax(ele).item()
                if id_word != PAD_ID and id_word != UNK_ID:
