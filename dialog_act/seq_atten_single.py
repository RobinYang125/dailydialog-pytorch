...
class Generator(nn.Module):
    ...
    def attention(self, hidden, encoder_outputs):
        ...
        return F.softmax(self.final_attn, dim=1).unsqueeze(1)
            ...
            for idx, x_len in enumerate(x_length):
                ...
                if is_toy:
                    pad_x, eos_pad_y, go_pad_y, x_length_, y_length_, y_mask = \
                        ...
                        torch.LongTensor([x_length[idx]]), \
                        torch.LongTensor([y_length[idx]]), \
                        torch.ByteTensor([y_mask[idx]])
                else:
                    pad_x, eos_pad_y, go_pad_y, x_length_, y_length_, y_mask = \
                        torch.cuda.LongTensor([pad_x_batch[idx]]), \
                        torch.cuda.LongTensor([eos_pad_y_batch[idx]]), \
                        torch.cuda.LongTensor([go_pad_y_batch[idx]]), \
                        torch.cuda.LongTensor([x_length[idx]]), \
                        torch.cuda.LongTensor([y_length[idx]]), \
                        torch.cuda.ByteTensor([y_mask[idx]])
...
def main_test(is_toy=False):
    ...
    res_file = 'models_' + prefix + '/seq_torch_batch_results.txt'
    res = codecs.open(res_file, 'w')

    reply_file = 'models_' + prefix + '/seq_torch_batch_reply.txt'
    reply_f = codecs.open(reply_file, 'w')

    ans_file = 'models_' + prefix + '/seq_torch_batch_answer.txt'
