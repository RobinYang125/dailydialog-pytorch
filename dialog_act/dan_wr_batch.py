...
class DataLoader(object):
    ...
    def get_batch_data(self):
        ...
        sort_x_length = sorted(x_length, reverse=True)
        indexs = [x_length.index(ele) for ele in sort_x_length]

        x_batch, y_batch, x_act_batch, y_act_batch, y_length = \
            x_batch[indexs], y_batch[indexs], x_act_batch[indexs], y_act_batch[indexs], y_length[indexs]
        ...
        return np.asarray(self.pad_sentence(x_batch, np.amax(x_length))), np.asarray(sort_x_length), \
               np.asarray(self.eos_pad(y_batch, y_max_length)), \
               np.asarray(self.go_pad(y_batch, y_max_length)), np.asarray(y_length), \
...
class Generator(nn.Module):
    ...
    def forward(self, encoder_input, encoder_input_lens, target_input, target_input_lens, is_test=False):
        ...
        if is_test:
            emb = self.embedding_layer(torch.cuda.LongTensor([[GO_ID for _ in range(batch_size)]]))
        outputs = []
...
def maskNLLLoss(inp, target, mask, rewards):
    nTotal = mask.sum()
    crossEntropy = -torch.log(torch.gather(inp, 1, target.view(-1, 1))).cuda() * rewards
    loss = crossEntropy.masked_select(mask).mean()
    return loss, nTotal.item()

def maskNLLLossVal(inp, target, mask):
    nTotal = mask.sum()
    crossEntropy = -torch.log(torch.gather(inp, 1, target.view(-1, 1))).cuda()
    loss = crossEntropy.masked_select(mask).mean()
    return loss, nTotal.item()
...
def main_train(is_toy=False):
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        for bn in range(data_loader.batch_num + 1):
            ...
            outputs_ids = []
            for out_sen_ten in outputs:
                out_ids = []
                for out_word in out_sen_ten:
                    out_id = torch.argmax(out_word, dim=0)
                    out_ids.append(out_id)
                outputs_ids.append(out_ids)

            out_len = [len(out) for out in outputs_ids]
            max_out_len = max(out_len)

            outputs_ids = data_loader.pad_sentence(outputs_ids, max_out_len)

            outputs_ids = torch.cuda.LongTensor(outputs_ids)

            # act
            act_outputs, act_rewards = act(outputs_ids, True)

            final_act_rewards = act_rewards[:, :, y_act_batch]

            print(act_rewards.shape)
            print(y_act_batch.shape)
            print(final_act_rewards.shape)

            n_total = 0

            for jdx in range(y_seq_len):
                mask_loss, nTotal = maskNLLLoss(outputs[jdx].view(batch_size, -1),
        ...
        for _ in range(data_loader.val_batch_num + 1):
            ...
            for jdx in range(y_seq_len):
                mask_loss, nTotal = maskNLLLossVal(outputs[jdx].view(batch_size, -1),
                                                eos_pad_y[:, jdx], y_mask[:, jdx])
