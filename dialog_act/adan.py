...
class DAN(nn.Module):
    ...
    def __init__(self, embeddings):
        ...
        self.h1_layer = torch.nn.Linear(self.embedding_size, layer1_units)
        self.h2_layer = torch.nn.Linear(layer1_units, self.act_class_num)
    ...
    def forward(self, x_batch):
        ...
        h1 = self.h1_layer(dense_inputs)
        h2 = self.h2_layer(h1)
        out = F.softmax(h2, dim=1)
        return out
...
def main_train(is_toy=False):
    ...
    # model
    if is_toy:
        model = DAN(data_loader.embedding_matrix)
    else:
        model = DAN(data_loader.embedding_matrix).cuda()
