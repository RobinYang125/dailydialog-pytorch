...
class Encoder(nn.Module):
    ...
    def forward(self, encoder_input, encoder_input_lens):
        ...
        return encoder_outputs, results, encoder_hidden
...
class Decoder(nn.Module):
    ...
    def forward(self, target_input, target_input_lens, encoder_hidden, is_test=False):
        target_embed = self.embedding_layer(target_input)
        # target_packed = torch.nn.utils.rnn.pack_padded_sequence(target_embed, target_input_lens,
        #                                                          batch_first=True)
...
def main_train(is_toy=False):
    ...
    if is_toy:
        encoder = Encoder(data_loader.embedding_matrix)
        # generator.load_state_dict(torch.load(model_prefix%'2'))
    else:
        encoder = Encoder(data_loader.embedding_matrix).cuda()
        encoder.load_state_dict(torch.load('models_encoder_classify/adan_para_40.pkl'))
        decoder = Decoder(data_loader.embedding_matrix).cuda()
    ...
    for epoch in range(EPOCH_SIZE):
        ...
        for bn in range(data_loader.batch_num + 1):
            ...
            encoder_outputs, results, encoder_hidden = encoder(pad_x, x_length_)
            outputs = decoder(go_pad_y, y_length_, encoder_hidden)
        ...
        torch.save(encoder.state_dict(), checkpoint_dir + 'encoder_para_' + str(epoch + 1) + '.pkl')
        torch.save(decoder.state_dict(), checkpoint_dir + 'decoder_para_' + str(epoch + 1) + '.pkl')
...
def response(query, reply, is_toy=False):
    ...
    pad_x_batch, x_length, \
        eos_pad_y_batch, go_pad_y_batch, y_length = data_loader.get_case(query, reply)
...
def main_test(is_toy=False):
    ...
    res_file = 'models_' + prefix + '/encoder_decoder_results.txt'
    res = codecs.open(res_file, 'w')

    reply_file = 'models_' + prefix + '/encoder_decoder_reply.txt'
    reply_f = codecs.open(reply_file, 'w')

    ans_file = 'models_' + prefix + '/encoder_decoder_answer.txt'
    ...
    if is_toy:
        ...
        encoder = Encoder(data_loader.embedding_matrix).cuda()
        decoder = Decoder(data_loader.embedding_matrix).cuda()
    encoder.load_state_dict(torch.load('models_encoder_decoder/encoder_para_4.pkl'))
    decoder.load_state_dict(torch.load('models_encoder_decoder/decoder_para_4.pkl'))
    print('Model has been restored')
...
def get_test_act():
    ...
    res_file = 'models_' + prefix + '/dan_wr_acts.txt'
    res = codecs.open(res_file, 'w')
...
if __name__ == '__main__':
    ...
    # response('what for ?', 'i am not sure')
